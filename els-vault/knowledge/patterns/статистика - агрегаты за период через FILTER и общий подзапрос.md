...
Выполнение графика ТО живёт в этом же модуле, но своим запросом: у него
другая таблица (`planned_to`, а не `order`) и другая единица счёта. Общими
оказались `MonthPeriod` с полуинтервалом, приём с `FILTER` и применение
области видимости одной точкой — `object_scope_filter`, потому что объект
там присоединён явно. Подзапрос `_breakdowns_query` там не при чём.

Просроченные ТО и топ сотрудников, скорее всего, повторят ту же развилку:
период и область общие, запрос свой.
