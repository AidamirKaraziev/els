...
Пошагово: [docs/server-deploy.md](docs/server-deploy.md).

Коротко: сервер ничего не собирает. Образы собирает CI и кладёт в `ghcr.io`, на сервере остаётся `git pull && make prod-deploy`. Сборка фронта требует несколько гигабайт RAM, на маленьком VPS она падает по OOM.
