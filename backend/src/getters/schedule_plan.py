...
def detect_anchor(
    *,
    ...
    previous_type_acts: Dict[int, Optional[int]],
) -> Optional[int]:
    """Восстановить якорь цикла по графику прошлого года.
    ...
    ТО всех заполненных месяцев прошлого года совпадают с программой.

    Ответ отдаётся, только если сдвиг **один**. Ни одного — прошлый год
    расставлен не по этой программе; несколько — по нему цикл не опознать
    (например, заполнен один месяц, а ТО1 в программе стоит на восьми
    позициях). В обоих случаях якорь обязан назвать человек: молча выбрать
    за него — значит сдвинуть весь год и не сказать об этом.
    """
    known = {
        month: type_act_id
        for month, type_act_id in previous_type_acts.items()
