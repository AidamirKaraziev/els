...
Пока здесь одна ручка — предпросмотр. Он ничего не пишет в базу: человек
сначала смотрит, что ляжет в двенадцать месяцев, и только потом утверждает
график. Создание (`POST /planned-to/generate/`) придёт следующим этапом и
ляжет в этот же модуль.
...
from src.crud.crud_schedule_plan import months_of_year
...
from src.schemas.schedule_plan import SchedulePreview
