...
@router.get(
    ...
    description=(
        ...
        "`defects_count` — сколько внутренних дефектных актов заведено на "
        "объект за год ленты, то же число, что у значка в карточке объекта. "
        "Считается здесь, а не отдельной ручкой на каждую строку: страница "
        "ленты — полсотни объектов.\n\n"
        ...
        "`object_id` оставляет одну строку — это лента в окне графика "
        "объекта. Отдельной ручки под неё нет намеренно: состояние клетки "
        "считает сервер, и второе место, где оно считается, разошлось бы с "
        "первым."
    ),
    tags=TAGS,
)
def schedule_rows(
    ...
    object_id: int = Query(
        None,
        title="Только этот объект",
        description=(
            "Одна строка вместо ленты — для окна графика объекта. Область "
            "видимости действует и здесь: чужой лифт даёт пустую выдачу."
        ),
    ),
