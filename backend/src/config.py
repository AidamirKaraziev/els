...
class Settings(BaseSettings):
    ...
    # Ссылка на файл, которую открывают в новой вкладке. Токен уезжает в адрес
    # и оттуда в журнал nginx, поэтому срок минимальный: минуты хватает, чтобы
    # браузер успел сходить по ссылке, и мало, чтобы ей кто-то воспользовался
    # из журнала. Картинки на странице грузятся тем же токеном, поэтому меньше
    # минуты ставить нельзя — медленная страница не успеет отрисоваться.
    FILE_TOKEN_EXPIRE_SECONDS: int = 60

    # Отдавать ли файлы через `X-Accel-Redirect`: бэкенд проверяет доступ и
    # отвечает заголовком, байты с диска отдаёт nginx из внутреннего
    # `location`. Так сохраняется и причина, по которой раздачу вынесли в
    # nginx (на одном ядре картинка через uvicorn занимает единственный
    # рабочий процесс), и проверка доступа, которая до этого на живом стеке
    # не выполнялась вовсе.
    #
    # По умолчанию выключено: `make dev` и тесты работают без nginx, и там
    # заголовок никто не читает — ответ пришёл бы пустым. Включается в
    # docker-compose, откат — та же переменная, без правки конфига.
    X_ACCEL_REDIRECT: bool = False

    #: Внутренний префикс, по которому nginx отдаёт файлы. Снаружи не
